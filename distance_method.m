 function result = distance_method(tilesAcross,method, alpha)
% method=1 without color correction,
% methode =2 Manhatan distance
% methode =3 Chebyshev distance
% methode =4 Euclidean distance
...
% color matching 
 ...
 for i=1:tilesDown
     for j=1:tilesAcross
        mosaic_man {i,j}(:,:,1)= alpha.*double(array(i,j,1))+ ((1-alpha).*double(mosaic_man{i,j}(:,:,1)));
        mosaic_man {i,j}(:,:,2)= alpha.*double(array(i,j,2))+ ((1-alpha).*double(mosaic_man{i,j}(:,:,2)));
        mosaic_man {i,j}(:,:,3)= alpha.*double(array(i,j,3))+ ((1-alpha).*double(mosaic_man{i,j}(:,:,3)));
    end
 end
...
if method == 2
    result = cell2mat(mosaic_man);
end

if method == 3
    result = cell2mat(mosaic_che);
end

if method == 4
    result = cell2mat(mosaic_euc);
end
